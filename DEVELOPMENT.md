...
- [x] **Security review** — rate limiting on auth (signup 5/min, login 10/min per IP), chat (15/min per user), trades (30/min per user), admin password (5/min per user), lobby creation (5/5min per user). Max 10,000 shares per trade. Password max length 128. Request body max 50KB. Security headers (X-Content-Type-Options: nosniff, X-Frame-Options: DENY). Sanitize function strips control characters. XSS audit passed — all user content sanitized or validated to safe character sets. SQL injection safe — all queries use parameterized bindings.
- [x] **Native Swift wrapper apps** — Xcode project in `BullRunApp/` folder. Single multiplatform target builds for macOS, iOS, and iPadOS. Uses WKWebView in SwiftUI to load the BullRun web app. Features: configurable server URL via gear icon, friendly error page when server is unreachable, dark theme background, App Transport Security configured for localhost HTTP. Open `BullRunApp.xcodeproj` in Xcode, select "My Mac" or an iOS simulator, and press Run.
